...
const ExploreStack: React.FC<ExploreStackProps> = ({ onMatch }) => {
    ...
    if (!currentProfile) {
        return (
            <div className="flex flex-col items-center justify-center h-full pb-20 p-8 text-center animate-fade-in">
                ...
                <p className="text-white/60 mb-8">You've seen all the profiles nearby. Check back later for more.</p>
                <button 
                    onClick={() => {
                        setProfiles(MOCK_EXPLORE_PROFILES);
                        setCurrentIndex(0);
                    }}
    ...
    return (
        <div className="relative h-full w-full overflow-hidden p-4 pb-24 flex flex-col">
            ...
            <div className="relative flex-1 w-full max-w-sm mx-auto">
                ...
                {nextProfile && (
                    <div className="absolute inset-0 rounded-3xl overflow-hidden bg-surface-dark scale-[0.95] translate-y-4 opacity-60">
                         <div 
                ...
                {/* Current Card */}
                <div 
                    className={`absolute inset-0 rounded-3xl overflow-hidden bg-black shadow-2xl transition-transform duration-300 ease-out border border-white/10
                        ${lastDirection === 'left' ? '-translate-x-[150%] rotate-[-10deg]' : ''}
                        ${lastDirection === 'right' ? 'translate-x-[150%] rotate-[10deg]' : ''}
                    `}
                >
                    <div 
                        className="absolute inset-0 bg-cover bg-center"
